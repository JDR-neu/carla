...
def callback(traj):
    global g_world, g_local_planner, g_target_speed
    ...
    if isinstance(g_world.player, carla.Vehicle):
        if traj is not None:
            ...
            for pt in traj.discretized_trajectory_:
                target_location = Location(pt.path_point_.x_, pt.path_point_.y_, 120.0)
                target_waypoint = g_world.world.get_map().get_waypoint(target_location)
                control = g_local_planner._vehicle_controller.run_step(g_target_speed, target_waypoint)
                g_world.player.apply_control(control)
                g_target_speed = g_target_speed + 0.5
                if g_target_speed > 90.0:
                    g_target_speed = 40.0


def listener():
    rospy.init_node('carla_basic_agent', anonymous=False)
...
g_local_planner = LocalPlanner(g_world.player)
